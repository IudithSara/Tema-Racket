...
(define (build-stable-matches-stream pref-stream)
  (define (update-matches smp previous-solution)
    (stable-match? smp (get-men smp) (get-women smp)))
  (letrec ((previous-solution (stream-first (stream-map update-matches pref-stream (stream-cons '() '())))))
    (stream-cons previous-solution (stream-map update-matches pref-stream previous-solution))))
