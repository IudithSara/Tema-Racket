...
(define (get-unstable-couples engagements mpref wpref)
  (let loop ((engs engagements) (unstable-couples '()))
    (if (null? engs)
        unstable-couples
        (let* ((woman (car (car engs)))
               (man (cdr (car engs))))
          (if (better-match-exists? man woman (get-pref-list mpref man) wpref engagements)
              (loop (cdr engs) (cons (car engs) unstable-couples))
              (loop (cdr engs) unstable-couples))))))
...
(define (gale-shapley mpref wpref)
  (engage '() (get-men mpref) wpref mpref))
...
(define (get-couple-members pair-list)
  (append-map (lambda (p) (list (car p) (cdr p))) pair-list))