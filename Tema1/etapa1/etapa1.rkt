...
(define (get-pref-list pref person)
  (if (null? pref)
      ...
      (if (equal?  (car (car pref)) person) 
          (cdr (car pref))  ;iau ultimele elem din prima pereche
...
(define (preferable? pref-list x y)
  (and (not (null? pref-list)) ;daca e goala return f
       (or (equal? (car pref-list) x) ;x preferat fata de y return t
           (and (not (equal? (car pref-list) y)) ;y preferat fata de x return f
