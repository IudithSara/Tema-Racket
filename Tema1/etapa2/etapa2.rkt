...
(define (get-pref-list pref person)
  ((lambda (person-pref)
     (if (null? person-pref)
         '()
         (cdr (car person-pref))))
    ; obtin lista cu toate perechile in care persoana
    ; data apare pe prima pozitie si e egala cu pers cautata
   (filter (lambda (x) (equal? (car x) person)) pref)))
...
(define (find-first p L)
  (cond ((null? L) #f)
        ((p (car L)) (car L)) ; daca e primul, returnez primul
...
(define (get-partner engagements person)
  (define (get-partner-helper couple)
  ; verific daca pe prima poz e pers cautata
    (if (eq? (car couple) person) 
        (cdr couple)
        #f))
  (let ((result (find-first get-partner-helper engagements)))
    (if result
        (cdr result)
        #f)))
...
(define (change-first p L val)
  (cond ((null? L) '())
        ; iau prima val din L si o inlocuiesc cu val
        ; cons cu prima val schimbata si restul listei
...
(define (update-engagements engagements p1 p2)
  (if (get-partner engagements p1)
      (change-first (lambda (couple) (eq? (cdr couple) (get-partner engagements p1)))
                    engagements
                    (cons p1 p2))
      engagements))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (if (null? p1-list)
      ...
      (if (equal? (car p1-list) p2)
          #f
          (or (preferable? (get-pref-list pref2 (car p1-list)) p1 (get-partner engagements (car p1-list)))
...
(define (stable-match? engagements mpref wpref)
  (helper-stable-match? engagements mpref wpref engagements))

(define (helper-stable-match? engagements mpref wpref aux_engagements)
  (if (null? (cdr engagements))
      (not (better-match-exists? (cdr (car engagements)) (car (car engagements)) (get-pref-list mpref (cdr (car engagements))) wpref aux_engagements))
      (and (not (better-match-exists? (cdr (car engagements)) (car (car engagements)) (get-pref-list mpref (cdr (car engagements))) wpref aux_engagements))
           (helper-stable-match? (cdr engagements) mpref wpref aux_engagements))))